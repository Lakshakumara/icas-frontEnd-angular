...
export class LoadDataSource extends DataSource<any> {
  ...
  filterData(claimType: string, year = 0, empNo: string = '', claimStatus: string,
    filter = this.filter, sortDirection = 'asc', pageIndex = this.paginator?.pageIndex, 
    pageSize = this.paginator?.pageSize) {
    this.loadingSubject.next(true);
    return this.auth.getAllClaims(claimType, year, empNo,
       claimStatus, filter, sortDirection, pageIndex, pageSize)
  }
    ...
    return this.auth.getClaim(claimId)
      .then((claim) => {
        ...
        x.claimData.forEach((d: any) => {
          ...
          if (d.claimDataStatus == 'Rejected') status = "Rejected - " + d.rejectRemarks;
          else if (d.claimDataStatus == 'Deducted') status = "Deducted - Rs. " + d.deductionAmount;
          else if (d.claimDataStatus == 'Approved') status = (d.remarks == null) ? "Approved " : "Approved - " + d.remarks;
          else status = "Other"
          if(d.scheme != null)
